function [acc_base, acc_finetune, acc_fix_f, acc_fix_gf] = newsgroups_experiment(input_layer_size,hidden_layer_size,max_iter,lambda,m_S,m_T,labels_S,labels_T)
%single newsgroups experiment
...
%load data
[X,y]=newsgroups_preprocess(input_layer_size);
...
X_T_train = X_T(1:m_T,:);
y_T_train = y_T(1:m_T);
X_T_test = X_T((m_T+1):(4*m_T),:);
y_T_test = y_T((m_T+1):(4*m_T));
